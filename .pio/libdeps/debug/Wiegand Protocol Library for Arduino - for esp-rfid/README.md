...
On `begin` you can pass an option to return the parity bits. Often the parity bits are included in the serial number of the cards. Having the option to return it means that you can support both ways of operation.

Example:

```
wg.begin(pinD0, pinD1, removeParityBitsBoolean);
```
